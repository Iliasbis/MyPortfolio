    ...
    return (
        <nav className='flex fixed justify-between w-full px-6 py-6 items-center shadow-md shadow-gray-300 bg-slate-100 z-50'>
            <h2 className='text-slate-500 cursor-pointer select-none self-center'>
                <span className='text-teal-500'>{logo}</span> ILias Bis{' '}
                <span className='text-teal-400'>{logoend} </span>{' '}
            </h2>
            <div
            ...
            >
                {links.map((l) => (
                    <React.Fragment key={l.name}>
                        ...
                        <li
                            className={`list md:mx-4 text-slate-500 select-none ${menu ? 'py-7 px-10 mx-auto md:py-0' : 'md:py-0 py-0'
                                } `}
                        >
                            <Link
                                to={l.link}
